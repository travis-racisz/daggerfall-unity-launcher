...
contextBridge.exposeInMainWorld("electron", {
  launchGame: () => {
    if (configFile.defaultConfig.executeable) {
      child_process.exec(
        `START ${configFile.defaultConfig.executeable}`,
    ...
    if (configFile.defaultConfig.gamePath) {
      ...
      child_process.exec(
        `START ${data?.defaultConfig.gamePath}\\DaggerfallUnity.exe`,
        { shell: process.env.ComSpec || "cmd.exe" },
        function (err, stdout, stderr) {
          if (!err) {
            console.log(err);
    ...
    progress(request.get(url))
      ...
      .on("finish", () => {
        ...
        unzip.on("data", (data) => {
          ...
          window.postMessage([
            "showProgress",
            ((unzipProgress / size) * 100).toFixed(2),
          ]);
        });
        unzip.on("end", () => {
          fs.unlink(`${dir}/${file}`);
      ...
      .on("finish", () => {
        ...
        unzip.on("end", () => {
          const files = fs.readdirSync(
            `${dir}/DaggerfallUnity_Data/StreamingAssets/Mods`,
          ...
          if (files) {
            for (const file of files) {
              fs.renameSync(
                `${dir}/DaggerfallUnity_Data/StreamingAssets/Mods/${file.name}`,
                `${dir}/daggerfall-update/DaggerfallUnity_Data/StreamingAssets/Mods/${file.name}`,
                (err) => {
          ...
          const moveFilesToParentDir = fs.readdirSync(
            `${dir}/daggerfall-update`,
            { encoding: "utf8", withFileTypes: true }
          );
          if (moveFilesToParentDir) {
            for (const file of moveFilesToParentDir) {
              fs.moveSync(
                `${dir}/daggerfall-update/${file.name}`,
                `${dir}/${file.name}`,
                { overwrite: true }
              );
            }
